...
public class DbFH {
                ...
                if (!"sqlserver".equals(dbtype)) {
                    if ("yes".equals(remoteDB)) {        //数据库另外一台服务器上(和tomcat不在同一台服务器上)
                        ...
                        if ("mysql".equals(dbtype)) {
                            ...
                            commandStr = "FHOracleHyQ IMP " + username + "/" + password + " FILE=" + sqlFilePath + " LOG=" + sqlFilePath.replace("DMP", "") + "LOG FULL=Y"; //oracle还原命令语句(还原前，先需要手动删除表,在sql编辑器里面删除即可)
                        }
                        Socket ss = null;
                        DataOutputStream bb = null;
                        DataInputStream dat = null;
    ...
    public static List<Map<String, String>> getFieldParameterLsit(Connection conn, String table) throws SQLException {
        ...
        while (rs.next()){
            ...
            String fieldType = rs.getString("TYPE_NAME");//字段类型
            String fieldLength = rs.getString("COLUMN_SIZE");//字段长度
            String fieldSccle = rs.getString("DECIMAL_DIGITS")==null?"0":rs.getString("DECIMAL_DIGITS");//字段精度
            String remarks = rs.getString("REMARKS")==null?"":rs.getString("REMARKS");//字段备注
            System.out.println(column_name);
            System.out.println(fmap);
            fmap.put("fieldNanme", column_name);                            //字段名称
            fmap.put("fieldType", fieldType);                        //字段类型名称
            fmap.put("fieldLength", fieldLength);    //长度
            fmap.put("fieldSccle", fieldSccle);
            fmap.put("remarks", remarks);
            columnList.add(fmap);
        }
